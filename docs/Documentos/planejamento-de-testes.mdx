...
Os testes de sistema levam em conta a aplicação real, com todas as integrações em módulos e componentes realizadas (como bancos de dados). São utilizados para verificar se as ações realizadas pelo usuário na aplicação final possuem o resultado esperado, além de capturar bugs em um cenário de produção.

Os testes de sistema são escritos a partir da aplicação móvel, utilizando a API de testes de intrumentalização do sistema Android, que permite a inspeção das telas do aplicativo de maneira automatizada.
